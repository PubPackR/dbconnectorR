...
- **Alle vier abhaengigen Kennzahlen aendern sich mit**, nicht nur die
  Show-Up-Quote: `kpi_sales_termine_showup()`, `kpi_sales_vcs_stattgefunden()`
  (Grundgroesse 8), der SDR-Anker in `termine_stattgefunden()` und darueber
  `erster_vc_je_lead()` (Closing-Time, `n_closes_ohne_vc`). Vier Kennzahlen mit
  drei Antworten auf dieselbe Frage waeren genau der Ausgangszustand.
...
- **Wirkung wird vor dem Merge ausgezaehlt**, auf der Population nach dem
  Badge-Fix aus PR 43 (dieser ist gemergt, aber noch nicht installiert).
...
- Die Ansage an Sales; die entscheidet Moritz nach der Wirkungsmessung
...
- Show-Up-Quote je Monat vorher/nachher aus dem Messskript, siehe PR-Body.
- Terminierung-Tab: die Zahl gelegter Termine darf sich **nicht** aendern.
