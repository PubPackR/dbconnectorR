...
crm_update_lead_mails_background <- function(con) {
  ...
  mails_leads <- persons %>%
    ...
    dplyr::mutate(
      lead_first_name = if_else(
        mail_address_type == "office",
        lead_first_name,
        NA_character_
      ),
      lead_first_name = if_else(
  ...
  persons_new_match <- mails_leads %>%
    ...
    dplyr::mutate(
      mail_address_name_compressed = gsub("@\\S+", "", mail_address_name)
    ) %>%
    dplyr::mutate(
      mail_address_name_compressed = tolower(mail_address_name_compressed)
    ) %>%
    dplyr::distinct(
      mail_address_name,
      mail_address_name_compressed,
      crm_lead_id,
      mail_address_type,
      salutation,
      lead_first_name,
  ...
  not_matchable_mails <- persons_new_match %>%
    dplyr::filter(
      !grepl("\\.", mail_address_name_compressed) |
        grepl(pattern, mail_address_name_compressed)
    ) %>%
    dplyr::mutate(
      personal_mail = if_else(mail_address_type == "office", TRUE, NA)
    ) %>%
    dplyr::mutate(
      personal_mail = if_else(is.na(personal_mail), FALSE, personal_mail)
    ) %>%
    dplyr::mutate(
      salutation = if_else(
        mail_address_type == "office",
  ...
  new_personal_mails <- persons_new_match %>%
    ...
    tidyr::separate(
      mail_address_name_compressed,
      c("lead_first_name", "lead_name"),
      "\\."
    ) %>%
    dplyr::mutate(
      lead_first_name = if_else(
        nchar(lead_first_name) < 2,
        toupper(lead_first_name),
        tools::toTitleCase(lead_first_name)
      )
    ) %>%
    dplyr::mutate(
      lead_name = if_else(
  ...
  all_personal_mails <- new_personal_mails %>%
    ...
    dplyr::mutate(
      lead_first_name = trimws(
        gsub("\\([^)]*\\)", "", lead_first_name),
        which = "right"
      ),
  ...
  all_mails <- all_personal_mails %>%
    ...
    dplyr::mutate(
      mail_address_type = dplyr::coalesce(mail_address_type, "from_description")
    ) %>%
  ...
  Billomatics::postgres_upsert_data(
    con,
    "raw",
    "crm_lead_mail_address",
    data,
    match_cols = c("lead_id", "mail_address_name"),
    delete_missing = TRUE
  )
  ...
  all_mails_compact <- all_mails %>%
    ...
    tidyr::drop_na(mail_address_name) %>%
    dplyr::mutate(
      compact_mail = mail_address_name %>%
        tolower() %>%
        sub("\\+.*(?=@)", "", ., perl = TRUE) %>%
        gsub("@([a-z0-9\\-]+)\\.[a-z]{2,}$", "@\\1", ., perl = TRUE) %>%
        gsub("\\+", "", .) %>%
  ...
  Billomatics::postgres_upsert_data(
    con,
    "mapping",
    "crm_lead_msgraph_contact",
    matched_with_msgraph,
    match_cols = c("msgraph_contact_id", "crm_lead_id"),
    delete_missing = TRUE
  )
}